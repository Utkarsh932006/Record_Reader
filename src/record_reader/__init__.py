...
ISP_COLUMN_MAP = {
    "Airtel": {"circuit_id": "SI Number", "start": "SR Creation", "end": "Resolved Time", "sheet": None},
    "JIO": {"circuit_id": "Service Id", "start": "Creation Date & Time", "end": "Resolution Date & Time", "sheet": "Tickets"},
    # Add new ISPs here with their column names
}


def extract_ckt_id(host):
    m = re.search(r"CKT-(\d+)", str(host))
...
def match_by_overlap(noc_df, isp_df):
    ...
    for _, noc_row in noc_df.iterrows():
        ...
        if isp_df.empty or ckt is None:
            ...
            results.append({**noc_row, "ISP_Start": pd.NaT, "ISP_End": pd.NaT, "Match_Status": status})
            continue

        # Find ISP records for same CKT with overlapping time window
        ckt_isps = isp_df[isp_df["CKT_ID"] == ckt]
        overlaps = ckt_isps[(ckt_isps["ISP_Start"] < noc_end) & (ckt_isps["ISP_End"] > noc_start)]

        if not overlaps.empty:
            best = overlaps.iloc[0]
            results.append({**noc_row, "ISP_Start": best["ISP_Start"], "ISP_End": best["ISP_End"], "Match_Status": "Matched"})
        elif isp_name not in known_isps:
            results.append({**noc_row, "ISP_Start": pd.NaT, "ISP_End": pd.NaT, "Match_Status": "NOC Only"})
        else:
            results.append({**noc_row, "ISP_Start": pd.NaT, "ISP_End": pd.NaT, "Match_Status": "Unmatched"})
...
def build_sheet1(noc_df, master_sites):
    ...
    all_days = pd.date_range(report_month.start_time, periods=report_month.day, freq="D")

    rows = []
    for (date, site), group in noc_df.groupby([noc_df["Time"].dt.floor("D"), "Site"]):
        isps = group["ISP"].unique()
        isp1, isp2 = (isps[0] if len(isps) > 0 else ""), (isps[1] if len(isps) > 1 else "")
        ...
        rows.append({
            "Date": date.strftime("%d-%m-%Y"), "Site": site,
            "ISP1 Name": isp1, "ISP1 CKT ID": ckt1, "ISP1 Down (min)": down1,
            "ISP2 Name": isp2, "ISP2 CKT ID": ckt2, "ISP2 Down (min)": down2,
            "Actual Site Down (min)": actual_down, "Daily SLA %": (1440 - actual_down) / 1440,
        })

    df_result = pd.DataFrame(rows)

    # Zero-downtime injection
    existing = set(zip(df_result["Date"], df_result["Site"])) if not df_result.empty else set()
    zero_rows = [
        {"Date": d.strftime("%d-%m-%Y"), "Site": s,
         "ISP1 Name": "", "ISP1 CKT ID": "", "ISP1 Down (min)": 0,
         "ISP2 Name": "", "ISP2 CKT ID": "", "ISP2 Down (min)": 0,
         "Actual Site Down (min)": 0, "Daily SLA %": 1.0}
        for d in all_days for s in master_sites
    ...
    return df_result.sort_values(["Site", "_sort"]).drop(columns=["_sort"]).reset_index(drop=True)
...
def build_sheet2(matched_df):
    ...
    rows = [{
        "Site": r["Site"], "CKT ID": str(r.get("CKT_ID", "") or ""), "ISP Name": r["ISP"],
        "Total Downtime (min)": r.get("Downtime (min)", 0), "Match Status": r.get("Match_Status", ""),
        "NOC Start": fmt(r["Time"]), "NOC End": fmt(r["Recovery time"]),
        "ISP Start": fmt(r.get("ISP_Start")), "ISP End": fmt(r.get("ISP_End")),
    } for _, r in matched_df.iterrows()]
    df = pd.DataFrame(rows)
    return df.sort_values(["Site", "CKT ID"]).reset_index(drop=True) if not df.empty else df
...
def write_report(sheet1, sheet2, output_path):
    with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
        ...
        for ws, cols in [(ws1, ["ISP1 CKT ID", "ISP2 CKT ID"]), (writer.sheets["Circuit Details"], ["CKT ID"])]:
...
def main() -> None:
    ...
    print(f"  {len(noc_df)} alerts, {noc_df['Site'].nunique()} sites, ISPs: {sorted(noc_df['ISP'].unique())}")

    print("Loading ISP hardware logs...")
    isp_df = load_all_isp_files(DOCS_DIR)
    print(f"  {len(isp_df)} ISP records from: {sorted(isp_df['ISP_Name'].unique())}" if not isp_df.empty else "  No ISP files found")
